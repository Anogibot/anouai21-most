...
class MOST():
    ...
    def _build_domain_discriminator(self, x_mid, reuse=None, scope='domain_disc'):
        return build_domain_discriminator_template(x_mid, domain_layout=self.domain_layout,
                                                   c=self.data_loader.num_src_domain, reuse=reuse, scope=scope)
    ...
    def _fit_loop(self):
        print('Start training MOST model at', os.path.abspath(__file__))
        ...
        self.checkpoint_path = os.path.join(model_dir(), self.model_name, "saved-model",
                ...
                _, primary_loss, W_dist, src_loss_class_sum, src_loss_class_lst, src_loss_discriminator, src_acc_lst, trg_acc, src_domain_acc, mimic_loss = \
                    self.tf_session.run(
                        [self.train_teacher, self.phase1_loss, self.OT_loss, self.src_loss_class_sum,
                         ...
                         self.src_accuracy_lst, self.trg_accuracy, self.src_domain_acc,
                         self.mimic_loss],
                        feed_dict=feed_data
                    )
            else:  # train phase 2
    ...
    def save_value(self, step):
        ...
        self.tf_summary_writer.flush()
        print_list = ['test_acc', round(ema_acc * 100, 2),]
        print(print_list)
